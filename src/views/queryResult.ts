...
export class QueryResult {
  // view tracking vars
  public static currentView: QueryResult | undefined;
  private static _views: Map<string, QueryResult> = new Map<
    string,
    QueryResult
  >();
  ...
  public static render(
    ...
    const queryResult: QueryResult | undefined = QueryResult.reveal(
      context,
      documentUri,
    );
    ...
    if (queryResult === undefined) {
      if (!webviewPanel) {
        ...
        webviewPanel = QueryResult.createWebviewPanel(context, documentUri);
      } else {
      ...
      QueryResult.currentView = new QueryResult(
        context,
        webviewPanel,
        documentUri,
      );
    ...
    queryResult?.updateActiveQueryResultContext(context);
  }
  ...
  public static reveal(
    ...
    const queryResult: QueryResult | undefined = QueryResult._views.get(
      viewUri.toString(true),
    ); // skip encoding

    if (queryResult !== undefined) {
      // show loaded webview panel
      queryResult.reveal(context);
      QueryResult.currentView = queryResult;
      return queryResult;
    } else {
      QueryResult.clearActiveQueryResultContext(context);
    }
   ...
   * @param context Extension context.
   */
  public static clearActiveQueryResultContext(context: ExtensionContext) {
    QueryResult.currentView = undefined;
    commands.executeCommand('setContext', ViewContext.QueryResultActive, false);
  ...
  private static createWebviewPanel(
    ...
    const webviewPanel = window.createWebviewPanel(
      constants.QueryResultPanel, // webview panel view type
  ...
  /**
   * Creates new QueryResult webview panel instance.
   *
   * @param context Extension context.
  ...
  private constructor(
    ...
    QueryResult._views.set(this._viewUri.toString(true), this);
    ...
    this._disposables.push(
      window.onDidChangeActiveTextEditor((editor) => {
        if (editor && editor.document.uri.fsPath === this.documentUri.fsPath) {
          ...
          this.clearQueryResultContext(context);
          this.update(context);
        }
      }),
    );
  ...
  public dispose(context: ExtensionContext) {
    QueryResult.currentView = undefined;
    QueryResult._views.delete(this._viewUri.toString(true)); // skip encoding
  ...
  public reveal(context: ExtensionContext) {
    ...
    this.updateActiveQueryResultContext(context);
  }
  ...
  private getHtmlTemplate(context: ExtensionContext, webview: Webview): string {
    ...
    const htmlTemplate = readFileSync(
      this.getResourceUri(context, 'query-result.html').fsPath,
      'utf-8',
    );
    const stylesheetUri: Uri = this.getResourceUri(context, 'sql-preview.css');
    const scriptUri: Uri = this.getResourceUri(context, 'queryResult.js');
   ...
   * @param context Extension context.
   */
  private async updateActiveQueryResultContext(context: ExtensionContext) {
    commands.executeCommand('setContext', ViewContext.QueryResultActive, true);
    commands.executeCommand(
      'setContext',
      ViewContext.LastActivePrqlDocumentUri,
      this.documentUri,
   ...
   * @param context Extension context.
   */
  private async clearQueryResultContext(context: ExtensionContext) {
    commands.executeCommand('setContext', ViewContext.QueryResultActive, false);
    context.workspaceState.update('prql.sql', undefined);
  }
